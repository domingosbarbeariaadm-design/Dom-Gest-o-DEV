...
let supabaseClient=null,syncState='local',syncInFlight=false,lastSyncAt='',syncChannel=null,lastRemoteCheckAt='';
...
function initSupabase(){if(supabaseClient)return supabaseClient; if(!window.supabase||!SUPABASE_URL||!SUPABASE_ANON_KEY)return null; try{supabaseClient=window.supabase.createClient(SUPABASE_URL,SUPABASE_ANON_KEY);return supabaseClient}catch(e){console.warn('Supabase indisponível',e);return null}}
async function loadFromSupabase(){try{const c=initSupabase();if(!c)return null;const {data,error}=await c.from('app_state').select('data,updated_at').eq('id',SUPABASE_STATE_ID).maybeSingle();if(error||!data)return null;let state=data.data||null;if(state){state.updated_at=stateStamp(state)||data.updated_at||stampNow()}return state}catch(e){console.warn('Falha ao carregar Supabase',e);return null}}
async function saveToSupabase(state){try{const c=initSupabase();if(!c)return false;const stamped=ensureStateStamp({...state});const {error}=await c.from('app_state').upsert({id:SUPABASE_STATE_ID,data:stamped,updated_at:stamped.updated_at});if(error)throw error;lastSyncAt=stamped.updated_at;return true}catch(e){console.warn('Falha ao salvar Supabase',e);return false}}
...
function startRealtimeSync(){const c=initSupabase();if(!c||syncChannel)return;try{syncChannel=c.channel('domingos-app-state')
  .on('postgres_changes',{event:'*',schema:'public',table:'app_state',filter:'id=eq.'+SUPABASE_STATE_ID},payload=>{const remote=payload?.new?.data; if(remote) applyRemoteState(remote);})
