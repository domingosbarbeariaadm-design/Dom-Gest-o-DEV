...
function hasValidSupabaseConfig(){return Boolean(window.supabase&&SUPABASE_URL&&SUPABASE_ANON_KEY&&SUPABASE_URL.startsWith('http')&&SUPABASE_ANON_KEY.length>20)}
function warnSupabaseNotConfigured(){setSyncState('local');if(supabaseConfigWarningShown)return;supabaseConfigWarningShown=true;console.warn('Supabase não configurado');alert('Supabase não configurado')}
function initSupabase(){if(supabaseClient)return supabaseClient; if(!hasValidSupabaseConfig()){warnSupabaseNotConfigured();return null} try{supabaseClient=window.supabase.createClient(SUPABASE_URL,SUPABASE_ANON_KEY);return supabaseClient}catch(e){console.warn('Supabase indisponível',e);warnSupabaseNotConfigured();return null}}
...
async function testSupabaseConnection(){if(!hasValidSupabaseConfig()){warnSupabaseNotConfigured();return alert('Supabase não configurado')}try{const c=initSupabase();if(!c)return alert('Supabase não configurado');const {error}=await c.schema(SUPABASE_SCHEMA).from(SUPABASE_TABLE).select('id').eq('id',SUPABASE_STATE_ID).limit(1);if(error)throw error;alert('Conectado com sucesso')}catch(e){console.warn('Erro ao conectar Supabase',e);alert('Erro ao conectar')}}
const MENU=[
['agenda','📅 Agenda'],
['inicio','🏠 Dashboard'],
['cadastros','📁 Cadastros'],
