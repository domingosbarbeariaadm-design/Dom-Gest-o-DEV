...
const KEY='domingos_v17_corrigido_faltantes';const OLD=['domingos_v16_correcao_uso_real','domingos_v15_escopo_completo','domingos_v14_corrigido','domingos_v13_final','domingos_v12_update','domingos_v11_corrigido_ok'];function id(){return Date.now().toString(36)+Math.random().toString(36).slice(2)}function hours(){return{0:{open:false,start:'08:00',end:'12:00',lunch:'',lunchDuration:60},1:{open:true,start:'08:00',end:'20:00',lunch:'12:00',lunchDuration:60},2:{open:true,start:'08:00',end:'20:00',lunch:'12:00',lunchDuration:60},3:{open:true,start:'08:00',end:'20:00',lunch:'12:00',lunchDuration:60},4:{open:true,start:'08:00',end:'20:00',lunch:'12:00',lunchDuration:60},5:{open:true,start:'08:00',end:'20:00',lunch:'12:00',lunchDuration:60},6:{open:true,start:'08:00',end:'18:00',lunch:'12:00',lunchDuration:60}}}function msgDefault(){return{birthday:'Olá {cliente}, feliz aniversário!',schedule:'Olá {cliente}, seu horário foi agendado para {data} às {hora}, para {servicos}.',confirm:'Olá {cliente}, confirmando seu horário: {data} às {hora}.',reminder:'Olá {cliente}, lembrando do seu horário às {hora}.',charge:'Olá {cliente}, consta em aberto {valor}. Pendências: {historico}',promo:'Olá {cliente}, temos novidades na Domingos Barbearia!'}}function seed(){return{biz:{name:'Domingos Barbearia',owner:'Eduardo Dudu',phone:'',cnpj:'',address:'',logo:'',hours:hours(),messages:msgDefault()},users:[{id:id(),name:'Eduardo Dudu',role:'Administrador + Barbeiro',pass:'1234'}],clients:[],services:[{id:id(),name:'Corte',value:45,duration:40,loyalty:true,color:'#f2c94c'},{id:id(),name:'Barba',value:40,duration:30,loyalty:false,color:'#4c8df2'},{id:id(),name:'Combo',value:80,duration:70,loyalty:true,color:'#35c26b'},{id:id(),name:'Sobrancelha',value:15,duration:15,loyalty:false,color:'#d73d49'},{id:id(),name:'Pezinho',value:10,duration:10,loyalty:false,color:'#8c5d10'}],products:[],appointments:[],cash:[],receivables:[],payables:[],plans:[],subs:[],sales:[],internalUses:[],discountTypes:[],paymentFees:{Pix:0,Dinheiro:0,Debito:1.99,Credito:3.99},blocks:[]}}function loadDB(){let r=localStorage.getItem(KEY);if(r)return JSON.parse(r);for(let k of OLD){let o=localStorage.getItem(k);if(o){let d=JSON.parse(o);localStorage.setItem(KEY,JSON.stringify(d));return d}}return null}let db=loadDB()||seed(),current='agenda',last='agenda',weekOff=0,recFilter='todos',payFilter='todos',planTemp=[],priceItems=[{name:'Produto/insumo',cost:3}];

const APP_CONFIG=(typeof window.APP_CONFIG==='object'&&window.APP_CONFIG)?window.APP_CONFIG:{};
const SUPABASE_URL=APP_CONFIG.SUPABASE_URL||'';
const SUPABASE_ANON_KEY=APP_CONFIG.SUPABASE_ANON_KEY||'';
const SUPABASE_TABLE='app_state';
const SUPABASE_SCHEMA='public';
const SUPABASE_STATE_ID='global';
let supabaseClient=null,syncState='local',syncInFlight=false,lastSyncAt='',syncChannel=null,lastRemoteCheckAt='',supabaseConfigWarningShown=false;
function stateStamp(st){return String(st?.updated_at||st?.atualizado_em||st?.updatedAt||'');}
function stampNow(){return new Date().toISOString()}
function ensureStateStamp(st){if(!stateStamp(st))st.updated_at=stampNow();return st}
function updateSyncBadge(){let el=document.getElementById('syncStatus');if(!el)return;let m={synced:'Online sincronizado',saving:'Salvando na nuvem',local:'Sem conexão, usando dados locais',error:'Erro ao sincronizar'};el.innerText=m[syncState]||m.local;el.className='syncStatus '+syncState;if(lastSyncAt&&syncState==='synced')el.title='Última sincronização: '+new Date(lastSyncAt).toLocaleString('pt-BR');}
function setSyncState(st){syncState=st;updateSyncBadge()}
function getSupabaseConfigDiagnostics(){let errors=[];if(typeof window.APP_CONFIG==='undefined')errors.push('config.js não carregou (window.APP_CONFIG indefinido)');if(typeof window.APP_CONFIG!=='object'||!window.APP_CONFIG)errors.push('window.APP_CONFIG precisa ser um objeto global');if(!window.supabase||typeof window.supabase.createClient!=='function')errors.push('SDK do Supabase não carregou (window.supabase.createClient indisponível)');if(!SUPABASE_URL)errors.push('SUPABASE_URL está vazio');if(!SUPABASE_ANON_KEY)errors.push('SUPABASE_ANON_KEY está vazio');if(SUPABASE_URL&& !SUPABASE_URL.startsWith('http'))errors.push('SUPABASE_URL inválido: deve começar com http/https');if(SUPABASE_ANON_KEY&&SUPABASE_ANON_KEY.length<=20)errors.push('SUPABASE_ANON_KEY inválido: tamanho muito curto');return{ok:errors.length===0,errors}}
function hasValidSupabaseConfig(){return getSupabaseConfigDiagnostics().ok}
function supabaseDiagnosticMessage(prefix='Supabase não configurado'){const d=getSupabaseConfigDiagnostics();if(d.ok)return prefix+': sem erros de configuração detectados';return prefix+':\n- '+d.errors.join('\n- ')}
function warnSupabaseNotConfigured(){setSyncState('local');const msg=supabaseDiagnosticMessage('Supabase não configurado');if(supabaseConfigWarningShown){console.warn(msg);return}supabaseConfigWarningShown=true;console.warn(msg);alert(msg)}
function initSupabase(){if(supabaseClient)return supabaseClient; if(!hasValidSupabaseConfig()){warnSupabaseNotConfigured();return null} try{supabaseClient=window.supabase.createClient(SUPABASE_URL,SUPABASE_ANON_KEY);return supabaseClient}catch(e){const msg='Supabase indisponível ao criar cliente: '+(e?.message||String(e));console.warn(msg,e);alert(msg);setSyncState('local');return null}}
async function loadFromSupabase(){try{const c=initSupabase();if(!c)return null;const {data,error}=await c.schema(SUPABASE_SCHEMA).from(SUPABASE_TABLE).select('data,updated_at').eq('id',SUPABASE_STATE_ID).maybeSingle();if(error||!data)return null;let state=data.data||null;if(state){state.updated_at=stateStamp(state)||data.updated_at||stampNow()}return state}catch(e){console.warn('Falha ao carregar Supabase',e);return null}}
async function saveToSupabase(state){try{const c=initSupabase();if(!c)return false;const stamped=ensureStateStamp({...state});const {error}=await c.schema(SUPABASE_SCHEMA).from(SUPABASE_TABLE).upsert({id:SUPABASE_STATE_ID,data:stamped,updated_at:stamped.updated_at});if(error)throw error;lastSyncAt=stamped.updated_at;return true}catch(e){console.warn('Falha ao salvar Supabase',e);return false}}
async function syncPullOnStart(){const local=loadDB();const remote=await loadFromSupabase();if(remote&&(!local||stateStamp(remote)>=stateStamp(local))){db=remote;localStorage.setItem(KEY,JSON.stringify(db));setSyncState('synced');return} if(local&&remote&&stateStamp(local)>stateStamp(remote)){setSyncState('saving');const ok=await saveToSupabase(local);setSyncState(ok?'synced':'local');if(ok)db=local;return} if(remote){db=remote;setSyncState('synced');return} setSyncState('local')}
async function syncPush(state){if(syncInFlight)return;syncInFlight=true;setSyncState('saving');const ok=await saveToSupabase(state);setSyncState(ok?'synced':'error');if(!ok)setTimeout(()=>setSyncState('local'),2200);syncInFlight=false}
function applyRemoteState(remote,{force=false}={}){if(!remote)return false;let remoteStamp=stateStamp(remote),localStamp=stateStamp(db);if(!force&&localStamp&&remoteStamp&&remoteStamp<localStamp)return false;db=remote;ensureStateStamp(db);localStorage.setItem(KEY,JSON.stringify(db));lastSyncAt=stateStamp(db)||stampNow();setSyncState('synced');render();return true}
async function refreshFromCloud({force=false}={}){const remote=await loadFromSupabase();if(!remote)return false;return applyRemoteState(remote,{force})}
function startRealtimeSync(){const c=initSupabase();if(!c||syncChannel)return;try{syncChannel=c.channel('domingos-app-state')
  .on('postgres_changes',{event:'*',schema:'public',table:SUPABASE_TABLE,filter:'id=eq.'+SUPABASE_STATE_ID},payload=>{const remote=payload?.new?.data; if(remote) applyRemoteState(remote);})
  .subscribe((status)=>{if(status==='SUBSCRIBED')setSyncState('synced')});
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){let now=Date.now();if(now-lastRemoteCheckAt>15000){lastRemoteCheckAt=now;refreshFromCloud();}}});
  window.addEventListener('online',()=>refreshFromCloud({force:true}));
}catch(e){console.warn('Falha ao iniciar realtime Supabase',e)}}
const $=x=>document.getElementById(x),money=v=>Number(v||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}),today=()=>new Date().toISOString().slice(0,10),num=v=>Number(String(v||0).replace(',','.'));function brDate(d){if(!d)return '';let p=String(d).split('-');return p.length==3?p[2]+'/'+p[1]+'/'+p[0]:d}function servicesText(a){return a&&a.service?a.service:'serviço agendado'}
