...
function getSupabaseConfig(){
  ...
  return {
    url:cfg.SUPABASE_URL||window.VITE_SUPABASE_URL||window.SUPABASE_URL||'',
    anonKey:cfg.SUPABASE_ANON_KEY||window.VITE_SUPABASE_ANON_KEY||window.SUPABASE_ANON_KEY||''
  };
}
...
function hasValidSupabaseConfig(){const cfg=getSupabaseConfig();return Boolean(window.supabase&&cfg.url&&cfg.anonKey&&cfg.url.startsWith('http')&&cfg.anonKey.length>20)}
...
function initSupabase(){if(supabaseClient)return supabaseClient; if(!hasValidSupabaseConfig()){warnSupabaseNotConfigured();return null} try{const cfg=getSupabaseConfig();supabaseClient=window.supabase.createClient(cfg.url,cfg.anonKey);return supabaseClient}catch(e){console.warn('Supabase indisponível',e);warnSupabaseNotConfigured();return null}}
